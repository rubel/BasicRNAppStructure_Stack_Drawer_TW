...
const DrawerList = [
  ...
  {icon: 'book', label: 'Library', navigateTo: ''},
];
const DrawerLayout = ({icon, label, navigateTo}) => {
  const navigation = useNavigation();
  return (
    <DrawerItem
      icon={({color, size}) => <Icon style={{ marginRight:-10}} name={icon} color={color} size={size} />}
      label={label}
      onPress={() => {
            ...
            <View style={styles.userInfoSection}>
              <View style={{flexDirection: 'row', marginTop: 15}}>
                ...
                <View style={{marginLeft: 10, flexDirection: 'column'}}>
                  <Title style={styles.title}>Md Rubel Hamza</Title>
                  <Text style={styles.caption} numberOfLines={1}>
                    rubel.xpress@gmail.com
                  </Text>
                </View>
              </View>
            </View>
